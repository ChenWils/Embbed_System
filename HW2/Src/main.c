...
uint8_t RemoteIP[] = {192, 168, 50, 234};
...
//  }
  ...
  while(1)
  {
    if(Socket != -1)
    {
	  ...
	  int len = sprintf(buffer,"{\"x\":%d,\"y\":%d,\"z\":%d,\"s\":%d}",((int)(x*10000))/10000,((int)(y*10000))/10000, ((int)(z*10000))/10000, sample_num);
	  ret = WIFI_SendData(Socket, buffer, sizeof(buffer), &Datalen, WIFI_WRITE_TIMEOUT);
